...
实际上, 我们打开form file生成的代码文件, 多了一段`connectSlotsByName`
...
在此之前, 我们对于Qt控件位置的控制, 都是通过绝对坐标的形式以code或者designer的形式来进行的. 但这里有一个很大的问题就是: 尽管在最开始, 由我们手动调整的控件看样子都是处在一个恰当的位置上, 但一旦窗口的大小被调整, 控件就会因为使用绝对坐标而无法自动的进行调整, 从而适应新的窗口大小.

于是, Qt引入了"布局管理器"的概念. 它会对其中的控件采用一种相对geometry的方式, 自行计算出其中控件的位置和大小参数, 并在窗口大小发生调整的时候重新计算刷新. 依据布局方式, Qt布局管理器可以分为 垂直布局, 水平布局, 网络布局, 表单布局者四种方式.

#### QVBoxLayout  

`QVBoxLayout  `是Qt中的垂直布局(管理器). 核心属性如下
...
布局管理器只负责布局, 不负责与用户交互, 因此, 它没有信号函数

下面我们就使用一下垂直布局管理器, 管理三个按钮, 先通过代码方式

![image-20251016095203018](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016095203018.png)

运行

![image-20251016095232290](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016095232290.png)

此时, 这三个按钮的geometry就会由布局管理器接管, 一方面, 这意味着它们可以依据窗口大小自适应, 另一方面, 这也意味着它们的geometry由layout控制, 我们调用相关接口将不起作用.

![image-20251016095758158](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016095758158.png)

![image-20251016095850601](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016095850601.png)

需要注意的一点是, 每个`widget`都只能设置一个布局管理器, 在designer上我们将会展示这一点.

在designer里, 我们可以拖入一个`Vertical Layout`, 并在其中放入三个按钮

![image-20251016100837843](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016100837843.png)

并且, 我们可以复制粘贴再生成一个

![image-20251016100941988](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016100941988.png)

这似乎和之前说的不太符合, 不要着急, 运行起来我们就会看到, 它和code那边的效果并不一样

![image-20251016101146983](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016101146983.png)

![image-20251016101205251](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016101205251.png)

code和designer对于layout的处理是不一样的, 对于code来说, 是直接创建一个layout, 而对于designer来说, 则是先创建一个widget, 然后在这个widget的里面再创建layout, 我们去form file中可以看到这一点

![image-20251016102325314](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016102325314.png)

所以之前说的是对的, 一个widget确实只能直接包含一个layout, designer 那里则是在两个子widget下面分别包含的, 与我们说的"一个widget只能直接包含一个layout"相吻合.

另外, 我们在designer里面, 还可以先创建三个按钮, 然后把它们加到一个layout里, 而不是像之前那样, 先创建一个layout, 然后再layout里面放按钮

![image-20251016103521042](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016103521042.png)

我们矩阵多选这三个按钮

![image-20251016103717620](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016103717620.png)

然后使用垂直布局

![image-20251016103741497](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016103741497.png)

另外我们还可以在这里微调一下属性, 我就不调了

![image-20251016104034160](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016104034160.png)

#### QHBoxLayout

`QHBoxLayout`, 即水平布局管理器, 核心属性如下:
...
![image-20251016111920898](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016111920898.png)

运行

![image-20251016111948017](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016111948017.png)

![image-20251016112003645](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016112003645.png)

尽管布局管理器之前不能并列, 但可以嵌套, 比如, 下面我们就写一个这样的程序, 它的主体是垂直布局, 里面有三个控件, 前两个是按钮, 第三个是由两个按钮组成的水平布局.

![image-20251016113044018](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016113044018.png)

运行

![image-20251016113103313](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016113103313.png)

![image-20251016113117937](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016113117937.png)

#### QGridLayout

Qt还提供了`QGrdiLayout`这种网格布局, 可以达到`M * N`的这种网格的效果. 

`QGridLayout`的核心属性和水平垂直布局差不多, 只不过多了一个方向的边距
...
![image-20251016121441861](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016121441861.png)

![image-20251016123336048](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016123336048.png)

![image-20251016121840349](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016121840349.png)

![image-20251016123653843](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016123653843.png)

![image-20251016124017261](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016124017261.png)

![image-20251016124029236](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016124029236.png)

需要注意的是, 那些坐标指示的都是相对坐标, 比如说单列你写成`(0, 1) (1, 1) (2, 1), (3,1)`也是相同的效果, 或者你也可以认为`0`列没有内容, 所以被省略了.

![image-20251016124620502](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016124620502.png)

![image-20251016124638777](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016124638777.png)

坐标是相对的, 比如这里改成`(100, 100)`他还是相同的效果

![image-20251016124749884](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016124749884.png)

-------

接下来我们看有关拉伸系数的问题

在此之前, 我们看到的控件布局都是均等的, 这一点, 不管是网格布局, 还是什么垂直水平布局都是相同的: 布局中的按钮大小尺寸都是一样的. 如果我们想要让它们不一样, 就要通过通过拉伸系数来描述每行或者每列的控件大小比例.

我们先看列拉伸系数.

我们将创建一个项目, 有六个按钮, 按照 2 * 3 的网格进行布局

![image-20251016231813166](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016231813166.png)

运行

![image-20251016231954525](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016231954525.png)

接下来我们就调整一下列拉伸系数

![image-20251016232352168](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016232352168.png)

运行

![image-20251016232415161](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016232415161.png)

我们调整的时候, 除非长不够了, 否则它都会保持这个比例

![image-20251016232620259](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016232620259.png)

![image-20251016232651118](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016232651118.png)

这里我们要特别说明的是拉伸系数0, 0表示不参与拉伸, 即固定尺寸

![image-20251016233229881](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016233229881.png)

![image-20251016233301026](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016233301026.png)

接下来我们去看行拉伸系数

没设置拉伸系数前

![image-20251016234112466](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016234112466.png)

运行

![image-20251016233918949](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016233918949.png)

现在我们加上拉伸系数

![image-20251016234152054](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251016234152054.png)

我们看到似乎没有什么效果

这是因为widget在垂直方向和水平方向的尺寸调整策略不同, 在列与列之间我们可以看到, 控件是尽量展开的, 而在行与行之间, 控件都是缩着的, 这就涉及到`widget`的一个属性, 叫做`SizePolicy  `, 这是一个枚举值, 可选的值如下

| 策略                     | 布局是否参考控件的推荐大小 | 是否可放大             | 是否可缩小         | 说明                                                         |
| ------------------------ | -------------------------- | ---------------------- | ------------------ | ------------------------------------------------------------ |
| `QSizePolicy::Ignored`   | 否                         | 是                     | 是                 | 忽略控件的推荐大小，只看最小/最大限制，可能被极度拉伸或压缩。 |
| `QSizePolicy::Minimum`   | 是                         | 是                     | 否                 | 至少满足一个合理的最小大小，可以更大，但不会更小。           |
| `QSizePolicy::Maximum`   | 是                         | 否                     | 是                 | 最多满足一个合理的最大大小，可以更小，但不会更大。           |
| `QSizePolicy::Preferred` | 是                         | 是                     | 是                 | 有推荐大小，布局会尽量贴近，但允许伸缩。                     |
| `QSizePolicy::Expanding` | 是                         | 是（主动争取更多空间） | 是                 | 在满足限制后，尽量多分配空间。                               |
| `QSizePolicy::Shrinking` | 是                         | 是                     | 是（主动接受缩小） | 愿意在必要时被压缩到比推荐大小更小，但不低于最小限制。       |

我们可以使用`setSizePolicy()`接口来对他们进行设置, 前一个参数是水平方向, 也就是列方向, 后一个就是垂直方向, 行方向的

![image-20251017000004662](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251017000004662.png)

![image-20251017000037712](https://md-wind.oss-cn-nanjing.aliyuncs.com/image-20251017000037712.png)
...
Qt 中同样有名为线程的存在. 也就是 `QThread`. 值得注意的是, 这`QThread`和C++的`Thread`风格不太一样. 实际上, `QThread`的风格是仿 Java 的. 一个很鲜明的点就是, 在C++中, 线程的入口函数一般都是以参数的形式输入线程中的, 然后线程内部再回调, 而`QThread`则是对线程的入口函数进行重写, 而这正是 Java 线程类的风格. C++之所以喜欢用回调这种方式, 是因为C++注重效率, 像"重写"这种多态操作比回调稍微多开销一些性能, 但是呢, Qt 又是 桌面级开发, 它面向的不是像服务端的其它客户端程序, 而是面向人, 直接与人互动, 所以这稍微多点性能损耗是完全可以接受的.
...
| `isRunning()`               | 若线程正在运行则返回 `true`，否则返回 `false`。              |
| `sleep()/msleep()/usleep()` | 使线程休眠，单位分别为：秒 / 毫秒 / 微秒。                   |
| `wait()`                    | 阻塞线程，直到以下任一情况发生：1) 与此 `QThread` 关联的线程执行完成（即从 `run()` 返回）。若线程已完成或未启动则返回 `true`；2) 已超过设定等待时长，若时间为 `ULONG_MAX`（默认）则永不超时；若超时则返回 `false`。其作用类似 POSIX 的 `pthread_join()`。 |
| `terminate()`               | 终止线程执行，是否立即结束取决于操作系统调度策略；调用 `terminate()` 后可使用 `QThread::wait()` 确认结束。 |
| `finished()`                | 当线程结束时发出该信号，可用于执行清理工作。                 |

#### 基本用法

下面, 我们就写一个十分简单的多线程程序: 创建一个副线程, 通过调用`sleep`成员函数, 实现类似于定时器的效果, 从而复现`QLCDNumber`的倒计时

首先, 既然是多态, 那么我们就再写一个类, 继承自 `QThread`

![image-20251107170302794](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251107170302794.png)

需要注意的是, 在`QLCDNumber`那里, 我们也特别提到过, Qt 为了界面中各个控件的安全, 是不允许副线程修改主线程中的控件的, 所以我们还是采用信号槽的机制, 就是时间到了, 就向`Widget`发送信号, 从而让主线程修改其中的控件属性.

![image-20251107171536211](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251107171536211.png)

![image-20251107172733750](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251107172733750.png)

那接下来, 我们在界面里创建一个`QLCDNumber`

![image-20251107171831490](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251107171831490.png)

![image-20251107172821846](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251107172821846.png)

![image-20251107172838220](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251107172838220.png)

---

在进行完上面的简单使用后, 我们来谈谈 Qt 中多线程的应用. 最开始, 我们学习多线程, 其核心目的是为了进行高性能服务器开发, 对于如今的服务器来说, 动辄就是什么 28 核, 56 核什么的, 甚至是双路 CPU, 即一个主板两个 CPU, 为了充分发挥这些硬件的性能, 我们就需要用多线程的方式提高程序运行的效率, 处理更多的客户端响应.

但是, Qt 是客户端开发, 所以 Qt 的多线程目的并不在此. 对于用户来说, 其所想要的是一个有着良好体验, 运行稳定的程序, 而不是让程序尽可能榨干性能, 榨干性能系统会变卡, 这样的话, 用户当然不乐意. 所以对于客户端来说, 多线程的主要作用不是榨干性能, 而是分流任务: 系统对于密集型IO任务会将执行流阻塞, 比如说, 客户端从服务端下载文件, 那按照逻辑来说, 你得把文件下载下来之后代码才能继续向下执行, 所以系统会把下载文件的线程阻塞, 等到什么时候, 文件下载完了, 再重新把它放回运行队列. 很明显, 主界面本身可不能阻塞, 阻塞那就没反应了, 可能它就会跳出一个系统弹窗: 检测到系统无反应, 你是继续等待, 还是直接关闭.

比如, 下图是我玩一款名叫"Stellaris", 中文名"群星"的单核游戏经常遇到的情况:

![image-20251107214641766](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251107214641766.png)

由于我打了创意工坊很多 MOD, 所以需要把他们加载到内存里, 这也算是一种密集型IO, 这样, 游戏加载的主界面就常常会卡住. 当然, 我听说 P社 曾经考虑过把群星多核化, 但是, 经过实验之后发现, 多核之后创意工坊的 MOD 就不兼容了, 所以一直都是单核. 要我选的话, 那我当然选能玩 MOD 的单核, 我又不是不讲理, 等一会儿就等一会吧. "一核有难, 八核围观", "CPU0 宣布你为「天灾」" 也算是 P社 玩家的一种自嘲了.

#### 线程安全

既然是多线程环境, 自然不可避免地涉及到所谓的"线程安全"问题. 关于其中的原理细节, 我们在学习 Linux 的时候已经说过, 在这里, 就不在原理上做赘述了.

其中最基本, 最多使用的线程安全手段, 便是加锁. 下面, 我们就来看看 Qt 里的锁长什么样, 有什么接口.

在 Qt 中, 互斥锁被称为 `QMutex`, 成员方法 `lock`加锁, `unlock`解锁. 下面, 我们就简单的使用一下.

我们将会写这样的一个程序: 创建两个副线程, 分别对同一个变量进行减减操作, 待两个副线程退出后, 由主线程回显该变量的值

首先是没有加锁的

![image-20251108125419332](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251108125419332.png)

![image-20251108125439462](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251108125439462.png)

![image-20251108125459662](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251108125459662.png)

![image-20251108125516320](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251108125516320.png)

最后结果如下

![image-20251108125922007](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251108125922007.png)

尽管没有 Linux 那样明显, 但还是能从现象中看出线程安全问题的, 一是从剩余票数上来看, 二是从打印内容本身来看

下面我们就来加锁

![image-20251108130742846](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251108130742846.png)

![image-20251108130806728](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251108130806728.png)

![image-20251108130832868](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251108130832868.png)

当然, 我们看到加锁引入了新的问题, 另一个线程似乎被"饿死"了. 在Linux那边, 我们也遇到了相同的问题, 我们说, 这是一种非正常状态, 实际上平时不会发生, 我们这里发生是因为没有非临界区的后续代码.

----

关于线程安全, 另外较常用到的就是条件变量或者信号量了, 在此我们就不写具体代码了: 原因也很简单, Linux 我们已经写过了, 而且代码还挺多. 所以这里我们的态度就是稍微提一提.

在 Qt 中, 条件变量叫做 `QWaitCondition`, 常用的接口如下

| QWaitCondition 成员 / 用法           | 对应 POSIX API / 概念                        | 说明                   |
| ------------------------------------ | -------------------------------------------- | ---------------------- |
| `wait(QMutex *mutex)`                | `pthread_cond_wait(&cond, &mutex)`           | 等待条件成立（无限期） |
| `wait(QMutex *mutex, ulong timeout)` | `pthread_cond_timedwait(&cond, &mutex, ...)` | 等待指定时间           |
| `wakeOne()`                          | `pthread_cond_signal()`                      | 唤醒一个等待线程       |
| `wakeAll()`                          | `pthread_cond_broadcast()`                   | 唤醒所有等待线程       |

使用时, 我们也要注意, 条件用 while 判断, 而不是 if, 具体我们也不说了

信号量叫做`QSemaphore`, 常用接口如下

| QSemaphore 成员 / 用法           | 对应 POSIX API / 概念           | 说明                             |
| -------------------------------- | ------------------------------- | -------------------------------- |
| `QSemaphore(int n = 0)`          | `sem_init(&sem, 0, n)`          | 创建信号量，初始可用资源数       |
| `acquire(int n = 1)`             | `sem_wait()` + (循环 n 次)      | 获取 n 个资源，不足则阻塞        |
| `tryAcquire(int n = 1)`          | `sem_trywait()` + (循环 n 次)   | 尝试获取资源，不足立即返回 false |
| `tryAcquire(int n, int timeout)` | `sem_timedwait()` + (循环 n 次) | 尝试在超时范围内获取资源         |
| `release(int n = 1)`             | `sem_post()` + (循环 n 次)      | 释放 n 个资源                    |
| `available()`                    | `sem_getvalue()`                | 获取当前可用资源数               |



----

关于线程安全, 还有一些其他锁, 比如读写锁, 具体我们就不说了, 它们的使用很简单也很好理解, 我们在 Linux 那里也说过了.

### 网络

和上面稍有不同的是, C++ 标准里面没有网络库, 所以这里我们直接照着`Qt`自己的`API`写就行了, 需要注意的是, 在这里我们主要介绍`API`, 不会说网络原理, 如果你没学过网络, 可以去我`github`的`linux`库的`Network`文件夹下去看对应的文档.

就像我们之前说网络原理讲的那样, 网络通信的基础支持都是内嵌到操作系统中的, 尽管各个操作系统具体的实现原理可能有所差异, 但效果上, 都是同样的协议栈, 而对于给应用层提供支持的传输层, 则有两大代表协议, 一个是`UDP`, 另一个就是`TCP`, 因此, 在下面, 我们就以此分类, 介绍并使用`Qt`封装的传输层接口.

#### UDP

在 Qt 中, 与`UDP`网络通信有关的类主要有两个, 一个是`QUdbSocket`, 另一个是`QNetworkDatagram`, 其中, `QNetorkDatagram` 是`UDP`每次网络通信时时的数据报类.
...
| `readyRead`                              | 信号 | 在收到数据并准备就绪后触发                   | 无（类似于 IO 多路转接的通知机制） |

相比于我们之前在 Linux 那边使用的原生接口, Qt 的一个天然优势是他有信号槽机制, 利用信号槽就可以自然地实现类似于多路转接的效果. 不用我们去捣鼓`epoll`了.

`QNetworkDatagram`的常用接口如下:

| 名称                                                         | 类型     | 说明                                                         | 对标原生 API                |
| ------------------------------------------------------------ | -------- | ------------------------------------------------------------ | --------------------------- |
| `QNetworkDatagram(const QByteArray&, const QHostAddress&, quint16)` | 构造函数 | 通过 `QByteArray`、目标 IP、目标端口号构造一个 UDP 数据报，通常用于发送数据 | 无                          |
| `data()`                                                     | 方法     | 获取数据报内部持有的数据，返回 `QByteArray`                  | 无                          |
| `senderAddress()`                                            | 方法     | 获取数据报中包含的对端 IP 地址                               | 无（`recvfrom` 包含此功能） |
| `senderPort()`                                               | 方法     | 获取数据报中包含的对端端口号                                 | 无（`recvfrom` 包含此功能） |

下面是`QString`和`QByteArray`的交互接口

`QString → QByteArray`

| 接口                                         | 说明                               | 备注                               |
| -------------------------------------------- | ---------------------------------- | ---------------------------------- |
| `QString::toUtf8()`                          | 以 UTF-8 编码转换为 `QByteArray`   | **最常用**，网络传输、文件 IO 推荐 |
| `QString::toLatin1()`                        | 以 Latin-1 编码转换为 `QByteArray` | 仅适合 ASCII/Latin-1 文本          |
| `QString::toLocal8Bit()`                     | 以本地编码转换为 `QByteArray`      | 依赖系统编码，不推荐跨平台使用     |
| `QString::toStdString()` / `.toStdWString()` | 转换为标准库字符串                 | 常用于与 STL 交互                  |

`QByteArray → QString`

| 接口                                         | 说明               | 备注                     |
| -------------------------------------------- | ------------------ | ------------------------ |
| `QString::fromUtf8(const QByteArray&)`       | 以 UTF-8 解码      | **网络和文件读取最常用** |
| `QString::fromLatin1(const QByteArray&)`     | Latin-1 解码       | ASCII/Latin-1 数据       |
| `QString::fromLocal8Bit(const QByteArray&)`  | 本地编码解码       | 不适合跨平台文本         |
| `QString::fromStdString(const std::string&)` | 从标准库字符串构造 |                          |

下面, 我们就基于`UDP`, 写两个项目, 分别表示服务端和客户端, 需要说明的是, Qt 一开始我们就说是前端开发, 而服务端都是命令行的, 我们这里写带`GUI`服务端纯粹是走个形式, 用用接口.

还是创建一个新的`widget`项目, 作为服务端, 为了有现象, 我们为其创建一个`QListWidget`作为之后受到数据的输出窗口.

![image-20251208002341223](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208002341223.png)

首先需要注意的是, 当前我们只使用了 Qt 中的`Core, Gui, Widgets`这三个模块, 并没有网络, 所以要加上`Network`. 如果你用的是`cmake`的话, 就应该在这里加一个`Qt6::Network`

![image-20251208002745200](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208002745200.png)

当然, 你们那边应该和我不一样, 这是因为我的`CMakeLists.txt`特别修改过, 用功能开关统一控制的.

如果你用的事`qmake`, 那就应该在第一行的那个`Qt += core gui`后面再追加一个`network`

之后就可以包含对应的头文件, `QUdpSocket, QNetworkDatagram`

![image-20251208010734423](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208010734423.png)

Creator 那边也有可能出现和我类似的问题: 找不到头文件, 这是因为语法分析器状态没有针对性地更新而造成的, 我们只需要重构--也就是重新编译项目即可.

![image-20251208003630591](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208003630591.png)

然后我们加个指针, 并且加个槽函数声明作为收到数据报的处理函数

![image-20251208010942476](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208010942476.png)

这里我们就选择本机所有的`IPv4`地址进行绑定(实际上, 我们的个人Windows电脑应该都是在私网里的--除非内网穿透, 否则我们只能本地回环)

![image-20251208004735541](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208004735541.png)

端口号就随便来个8080吧, 之前在 Linux 那边, 我也一般用80开头的.

那我们这里再写一个回调函数, 负责处理请求并返回应答

![image-20251208011833431](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208011833431.png)

![image-20251208013425772](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208013425772.png)

`std::format`是`C++20`版本的.

----

服务端写好了那就去写客户端, 同样的, 还是新建一个 widget 项目, 其实上我也可以把上面的项目改一改文件层次, 生成两个程序, 但 Creator 明显做不到, 所以这里还是重新创建一个项目了

对于客户端, 其UI结构主体上是三个部分: 一个`QListWidget`, 负责显示服务端和客户端相互传达的消息,  一个输入框, 负责读取用户的输入, , 一个按钮, 负责将输入框的内容包装成一份请求.

![image-20251208020003540](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208020003540.png)

此处, 我们把输入框和按钮用水平布局管理, 然后它们整体再用垂直布局

![image-20251208020120965](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208020120965.png)

![image-20251208020331071](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208020331071.png)

或许应该调整一下垂直方向上的拉伸系数

![image-20251208021244386](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208021244386.png)

然后我们再让水平布局里的两个组件垂直方向展开

![image-20251208021550139](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208021550139.png)

感觉比例不太对, 再调一下

![image-20251208021649111](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208021649111.png)

![image-20251208021702406](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208021702406.png)

在头文件, 我们把该加的头文件加上去, 还有服务端的地址

![image-20251208023550855](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208023550855.png)

自然, 按下发送按钮的时候发送, 因此我们也要写对应的槽函数

![image-20251208023745441](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208023745441.png)

![image-20251208024710201](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208024710201.png)

如果有数据发过来了, 那就同样是用信号槽来读取发回内容, 并写到列表框中

![image-20251208025252911](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208025252911.png)

两个运行一下

![image-20251208025609999](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208025609999.png)

![image-20251208025557293](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208025557293.png)

服务端的`ip, port`打印不怎么好, 可能是 std::format 匹配错类型了, 不是什么大事.

正如之前说过的那样, 网络协议栈在效果上都是一样的, 所以你用 Qt 的客户端是完全可以连上我们之前 Linux 那边写的服务端的, 只不过由于应用层协议不一样, 可能会直接关闭, 实际上, 一般就是 Linux 纯命令行环境做服务端, Qt 这种 `gui` 框架写客户端.

#### TCP

对于 `TCP` 来说, 核心类有两个: `QTcpServer, QTcpSocket`

`QTcpServer`用于监听端口, 功能上类似于我们 Linux 之前写的监听套接字

| 名称                                        | 类型 | 说明                                                         | 对标原生 API                    |
| ------------------------------------------- | ---- | ------------------------------------------------------------ | ------------------------------- |
| `listen(const QHostAddress&, quint16 port)` | 方法 | 绑定指定的地址和端⼝号，并开始监听。                         | bind + listen                   |
| `nextPendingConnection()`                   | 方法 | 从系统获取一个已经建立好的 TCP 连接，返回一个 `QTcpSocket` 用于与客户端通信。 | accept                          |
| `newConnection`                             | 信号 | 有新的客户端建立连接时触发（类似 IO 多路复用中的事件通知机制）。 | 无（但类似 `epoll` 的事件通知） |

`QTcpSocket`就是普通的连接

| 名称                       | 类型 | 说明                                                         | 对标原生 API                         |
| -------------------------- | ---- | ------------------------------------------------------------ | ------------------------------------ |
| `readAll()`                | 方法 | 读取当前接收缓冲区的所有数据，返回 `QByteArray`。            | read                                 |
| `write(const QByteArray&)` | 方法 | 将数据写入 socket。                                          | write                                |
| `deleteLater`              | 方法 | 将 socket 对象标记为无效，Qt 会在下一个事件循环中自动析构释放该对象。 | 无（但类似“半自动化垃圾回收”）       |
| `readyRead`                | 信号 | 有数据到达并准备就绪时触发。                                 | 无（但类似 IO 多路复用中的事件通知） |
| `disconnected`             | 信号 | 连接断开时触发。                                             | 无（但类似 IO 多路复用中的事件通知） |

这里注意一下 `deleteLater`，它的整体思路和我们之前基于 `epoll` 的做法可以说**类似，但实现机制又不太一样**。

区别在于：Qt 里的 `deleteLater` 属于**惰性删除**——它并不会立刻析构对象，而是把这个对象标记为“待删除”，等到**下一次事件循环**时再真正执行析构。

而我们之前在 `epoll` 版本里，用的是**智能指针**来管理会话对象的生命周期，通过**引用计数**来控制析构时机：
 我们会先在会话管理里把这个连接“注销”，相当于从会话层移除它；之后等所有持有该会话的智能指针都销毁、引用计数归零时，对象才会被真正析构。某种意义上，这也有点“延迟析构”的味道，但它的触发时机和 `deleteLater` 还是不太一样。

`deleteLater` 放到**下一轮事件循环**再析构，有一个很重要的好处：它可以尽量保证这个对象相关的所有回调、信号槽都已经跑完了——也就是说，不会出现这种危险情况：这边刚刚析构完对象，那边又因为某个信号触发，试图通过已经被销毁的指针去访问它，从而造成野指针问题。

-----

还是照旧, 我们先写服务端, 再写客户端.

对于服务端, 以往我们在  Linux 那边时, 为了防止单个连接的收发循环过程阻塞监听套接字获取新的连接, 最开始改成了多执行流的版本, 后来我们又学习了`epoll`控制服务端, 也实现了单执行流对于多个连接的收发, 但在这里, 我们既不用写多执行流, 也不用写`epoll`, 因为 `Qt` 的信号槽机制天然地在更高维度上实现了`epoll`基于事件驱动的核心内在, 所以用 Qt 写服务端还是很轻松的

![image-20251208164550182](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208164550182.png)

头文件那边就不贴了

![image-20251208201457437](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208201457437.png)

如果来了一个新连接, 首先回显一下地址端口, 然后对这个连接的`readyRead`信号准备对应的读方法槽函数

![image-20251208204725459](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208204725459.png)

严格来说，对于**每一个 TCP 连接**，我们都应该分配它自己独立的**接收缓冲区**和**发送缓冲区**，就像在 `epoll` 编程模型里常做的那样。原因在于 **TCP 是面向字节流的**：应用层所谓的“报文边界”在 TCP 层并不存在，因此我们一次 `read/recv` 读到的数据可能：

- **不足一个完整报文**（也就是常说的“半包”/不完整消息），这种情况下就需要把这部分数据先暂存在接收缓冲区里，等待后续数据到达后再拼成完整报文进行解析；
- **包含多个报文**（“粘包”），则需要从缓冲区中按协议规则拆分出多条消息依次处理。

不过这里为了简化实现，同时也考虑到这部分内容和 Qt 本身关系不大，我们先做一个假设：**每次 `readyRead` 触发时，收到的数据刚好就是一条完整报文**，因此暂时不引入每连接的缓冲区管理与拆包/组包逻辑。

另外，因为我们目前并没有对 `QTcpSocket` 做进一步封装，所以在处理可读事件时更适合使用 **lambda**，而不是直接写成 `Widget` 的成员槽函数。主要有两个原因：

1. 目前我们没有一个统一的容器来管理这些 `QTcpSocket*`；
2. 即使有容器，把读事件写成 `Widget` 的成员函数后，当 `readyRead` 触发时，我们仍然需要额外手段才能确定**到底是哪个连接**触发了这个信号。

因此，我认为更合理的做法是对 `QTcpSocket` 再做一层封装：一方面在封装类内部维护该连接的收发缓冲区，另一方面把“读事件处理”这类逻辑做成封装类的成员槽函数。这样每个连接的状态、缓冲区和处理逻辑都能自然地绑定在一起，结构也会更清晰。但还是因为同样的原因, 这里我们还是直接写了.

![image-20251208210639956](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208210639956.png)

接下来我们再为连接的断开信号增加对应的槽函数

![image-20251208213117925](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208213117925.png)

在这里释放连接对象时，我们必须调用它自己的成员函数 `deleteLater()`，而不能直接对 `QTcpSocket` 执行 `delete`。

原因有几点：

1. **Qt 是一个完整的框架**，`QObject` 及其子类（包括 `QTcpSocket`）的生命周期，往往和事件循环、信号槽机制、父子对象关系等内部机制紧密关联。我们并不清楚 Qt 在内部对这些对象还做了哪些管理。
2. 如果我们贸然手动 `delete`，有可能破坏 Qt 框架内部对该对象的预期流程，导致一些非常隐蔽、难以排查的问题。
3. 此外，直接 `delete` 也容易触发我们前面提到的那类风险：对象刚被销毁，某个尚未处理完的信号/回调却还在尝试访问它，从而产生野指针。

而 `deleteLater()` 会把对象标记为“待删除”，等到**下一次事件循环**中再真正销毁，这样就可以尽量确保与该对象相关的信号槽、回调等都已经处理完毕，再安全地释放资源。

接下来我们去写客户端

和以前一样, 我们直接用`UDP`的`UI`

![image-20251208221545435](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208221545435.png)

![image-20251208223112044](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208223112044.png)

然后我们去写发送按钮按下对应的发送槽函数和接收槽函数

![image-20251208224658061](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208224658061.png)

好了运行

![image-20251208225129962](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208225129962.png)

![image-20251208225202948](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251208225202948.png)
...
好的, 下面我们就创建一个新的 widget 项目

这次, 对于 `ui` 布局我们要稍微换一下, 我们把最大的 `listWidget`换成 `plainTextEdit`, `plainTextEdit`可以将原始的报文数据显示出来--因为我们做的是一个简易的浏览器, 所以收到的应答应该是一个 html 网页, 对于我们来说, 原始内容当然是更清楚的, 因此不用含有一定渲染功能的 `textEdit`. 另外, `lineEdit`现在输的是 `url`

![image-20251214140821192](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251214140821192.png)

![image-20251214140834690](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251214140834690.png)

![image-20251214142958571](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251214142958571.png)

![image-20251214143014122](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251214143014122.png)

这里我们就访问一下百度

![image-20251214143208383](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251214143208383.png)

![image-20251214143220674](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251214143220674.png)

当然, 浏览器其实是计算机行业皇冠上的明珠之一, 我们以后当然不会写浏览器, 因此, 在日后, 我们所收到的正文不一定是 html, 而是其它的表示方法.

### 多媒体

有时客户端可能需要音视频, 那此时, 我们就需要调用相应的接口, 来播放它们, 视频其实用的很少, 除非是特定的场景, 因此我们这里只看看音频

注意, 和之前的网络相同, 多媒体也有自己的模块, `multimedia`, `qmake`全小写, `cmake`首字母大写, 所以打开项目后, 我们需要添加对应的模块.

在 Qt 6 版本中, 推荐使用 `QMediaPlayer` 播放音频, (更早的版本主要用 `QSound`, 单实际上`QSound`并不好用)

| 接口                                      | 类型     | 必需性           | 说明                                           |
| ----------------------------------------- | -------- | ---------------- | ---------------------------------------------- |
| `QMediaPlayer(QObject *parent = nullptr)` | 构造函数 | 必需             | 创建播放器对象                                 |
| `setAudioOutput(QAudioOutput *output)`    | 方法     | **必需（音频）** | 绑定音频输出设备，否则无法出声                 |
| `setSource(const QUrl &source)`           | 方法     | **必需**         | 设置播放源（本地文件 / 资源 / URL）            |
| `play()`                                  | 方法     | **必需**         | 开始播放                                       |
| `pause()`                                 | 方法     | 常用             | 暂停播放                                       |
| `stop()`                                  | 方法     | 常用             | 停止播放并重置状态                             |
| `state()`                                 | 方法     | 常用             | 获取当前播放状态（Playing / Paused / Stopped） |
| `mediaStatus()`                           | 方法     | 常用             | 获取媒体加载状态                               |
| `error()`                                 | 方法     | 常用             | 获取错误码                                     |
| `errorString()`                           | 方法     | 常用             | 获取错误描述文本                               |

在这里我从网易云上随手下载了一份音频, 用作材料.

<!-- Typora / 本地阅读器可见 -->

<audio controls src="https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/Rick%20Wakeman%20-%20The%20Granary%20Canon.mp3"></audio>

<!-- GitHub 可见 -->
[🎵 点击播放音频](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/Rick%20Wakeman%20-%20The%20Granary%20Canon.mp3)

可能由于渲染问题, `Github` 那边看不到 Typora 那种播放器, 所以你可以直接从链接上直接下载

在这之后, 我们把它复制到项目目录下, 用`qrc`机制将其包含进去. `Create`的`qrc`怎么用我们已经说过了, 我这里还是老样子纯手写, 为了方便起见, 我给它取了别名.

![image-20251214154742224](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251214154742224.png)

![image-20251214155555886](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251214155555886.png)

![image-20251214155609011](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251214155609011.png)

由于播放一段歌曲需要一定的时间, 所以我们要把`player`放到全局里面, 否则的话, `player`会因为是局部变量析构过快实际上放不出来音频. 另外我们从上面的接口列表中也可以看到, `QMediaPlayer`必须要绑定一个可用的音频设备, 所以我们也需要包含对应的头文件及其对象

![image-20251214162321566](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251214162321566.png)

我们先看看当前系统中有哪些音频输出设备

![image-20251214161458073](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251214161458073.png)

![image-20251214161520560](https://wind-note-image.oss-cn-shenzhen.aliyuncs.com/image-20251214161520560.png)

实际中, 你可以根据查询到的设备内容列个下拉框 供用户进行选择, 但这里为了方便, 我们就选择下标为零的那个, 也就是这台机器的默认音频输出设备--耳机.
